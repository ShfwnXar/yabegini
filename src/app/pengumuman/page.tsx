      ...
      <div className="mx-auto max-w-7xl space-y-6 px-4 py-10">
        <section className="overflow-hidden rounded-3xl border border-emerald-100 bg-white/90 p-6 shadow-[0_20px_40px_rgba(15,139,76,0.12)] md:p-8">
          <div className="grid items-center gap-6 md:grid-cols-[1.3fr_0.7fr]">
            <div>
              ...
              <h1 className="mt-3 text-3xl font-extrabold text-gray-900 md:text-4xl">Pengumuman</h1>
              <p className="mt-2 max-w-2xl text-sm text-gray-600 md:text-base">
                Pengumuman resmi terkait jadwal, technical meeting, aturan lomba, dan informasi penting lainnya.
              </p>
              ...
              <div className="mt-5 flex flex-wrap gap-3">
                ...
                <Link href="/berita" className="rounded-xl bg-emerald-600 px-4 py-2 text-sm font-semibold text-white hover:bg-emerald-700">
                  Lihat Berita
                </Link>
              </div>
            </div>
        ...
        <section className="rounded-2xl border border-emerald-100 bg-white/90 p-5 shadow-sm">
          {announcements.length === 0 ? (
            <div className="text-sm text-gray-600">Belum ada pengumuman. Admin dapat menambahkan pengumuman dari panel admin.</div>
          ) : (
            <div className="space-y-4">
              {announcements.map((p) => (
                <article key={p.id} className="rounded-xl border border-emerald-100 bg-gradient-to-r from-white to-emerald-50/50 p-5">
                  ...
                  <div className="mt-4">
                    <Link href={`/pengumuman/${p.id}`} className="text-sm font-semibold text-emerald-700 hover:underline">
                      Baca selengkapnya
                    </Link>
                  </div>
                </article>
